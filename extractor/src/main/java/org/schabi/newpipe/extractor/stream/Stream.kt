 ...
 */(
    ...
    open fun equalStats(other: Stream?): Boolean {
        if ((other == null) || (format == null) || (other.format == null)) {
            return false
        }
        return (format.id == other.format.id) && (deliveryMethod == other.deliveryMethod
                ) && (isUrl == other.isUrl)
    }
    ...
    companion object {
        val FORMAT_ID_UNKNOWN: Int = -1
        val ID_UNKNOWN: String = " "
         ...
         *
         */
        val ITAG_NOT_AVAILABLE_OR_NOT_APPLICABLE: Int = -1
         ...
         */
        fun containSimilarStream(stream: Stream?,
                                 streamList: List<Stream?>): Boolean {
            if (Utils.isNullOrEmpty(streamList)) {
                return false
            }
            for (cmpStream: Stream? in streamList) {
