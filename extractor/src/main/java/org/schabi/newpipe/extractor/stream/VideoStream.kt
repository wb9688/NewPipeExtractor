...
class VideoStream private constructor(id: String,
     ...
     */
    // Fields for DASH
    var itag: Int = Stream.Companion.ITAG_NOT_AVAILABLE_OR_NOT_APPLICABLE
     ...
     * {@inheritDoc}
     */
    override val itagItem: ItagItem? = null
    ...
    () {
        ...
        fun build(): VideoStream {
            ...
            if (content == null) {
                throw IllegalStateException(("The content of the video stream has been not set "
            ...
            if (isVideoOnly == null) {
                throw IllegalStateException(("The video stream has been not set as a "
            ...
            return VideoStream(id!!, content!!, isUrl, mediaFormat, deliveryMethod!!, resolution!!,
    ...
    init {
        if (itagItem != null) {
            this.itagItem = itagItem
            itag = itagItem.id
            bitrate = itagItem.getBitrate()
            initStart = itagItem.getInitStart()
            initEnd = itagItem.getInitEnd()
            indexStart = itagItem.getIndexStart()
            indexEnd = itagItem.getIndexEnd()
            codec = itagItem.getCodec()
            height = itagItem.getHeight()
            width = itagItem.getWidth()
            quality = itagItem.getQuality()
            fps = itagItem.getFps()
        }
        this.resolution = resolution
        this.isVideoOnly = isVideoOnly
    ...
    public override fun equalStats(cmp: Stream?): Boolean {
        return (super.equalStats(cmp)
                && cmp is VideoStream
                && (resolution == cmp.resolution) && (isVideoOnly == cmp.isVideoOnly))
    }

    companion object {
        val RESOLUTION_UNKNOWN: String = ""
    }
}
