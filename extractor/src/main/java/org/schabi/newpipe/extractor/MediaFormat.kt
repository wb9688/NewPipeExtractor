 ...
 */
    ...
    companion object {
        private fun <T> getById(id: Int,
                                field: Function<MediaFormat, T>,
                                orElse: T): T {
            return Arrays.stream(entries.toTypedArray())
            .filter(Predicate ({ mediaFormat: MediaFormat -> mediaFormat.id == id}))
            .map(field)
            .findFirst()
            .orElse(orElse)
        }
 ...
 */
         fun getMimeById(id: Int): String? {
            return getById(id, Function ({ obj: MediaFormat -> obj.mimeType }), null)
        }
         /**
 ...
 */
         fun getFormatById(id: Int): MediaFormat? {
            return getById(id, Function ({ mediaFormat: MediaFormat? -> mediaFormat}), null)
        }
         /**
 ...
 */
         fun getFromSuffix(suffix: String): MediaFormat? {
            return Arrays.stream(entries.toTypedArray())
            .filter(Predicate ({ mediaFormat: MediaFormat? -> (mediaFormat!!.suffix == suffix)}))
            .findFirst()
            .orElse(null)
        }
    }
